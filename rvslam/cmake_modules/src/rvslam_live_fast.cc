...
Mat34 EulerToRot(const double roll, const double pitch, const double yaw) {
  Mat34 rot;
...
int main (int argc, char** argv) {
  ...
  for (int k = 0; k < FLAGS_num_capture; k++) {
    ...
    const double t = ((double)(end - begin) / CLOCKS_PER_SEC);
    ...
    Vec6 cur_pose_imu;
    cur_pose_imu << roll, pitch, yaw, 0, 0, 0;
    ...
    if (idx == FLAGS_start) {
      feat.Setup(FLAGS_feature, img_mat_ori, idx, FLAGS_reduce_size);
    } else {
      Vec6 origin;
      origin << 0, 0, 0, 0, 0, 0;
      //const Mat4 cur_pose_vo = ToMat4(VisualOdometer::ToPoseMatrix
      //                              (world.keyframes_[1].pose));

      Mat4 rel_rot;
      const Mat34 prev_rot = EulerToRot(prev_pose(0), prev_pose(1), prev_pose(2));
      const Mat34 cur_rot = EulerToRot(roll, pitch, yaw);
      rel_rot = ToMat4(cur_rot) * (ToMat4(prev_rot).inverse());
      feat.Process(FLAGS_feature, img_mat_ori, idx, latest_knum,
          FLAGS_reduce_size, feat.last_ftid(), rel_rot);
      prev_pose = cur_pose_imu;
    }
