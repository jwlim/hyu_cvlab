...
int main(int argc, char **argv) {
  ...
  for (int idx = FLAGS_start; idx <= FLAGS_end; idx += FLAGS_step) {
    ...
    if (idx == FLAGS_start) {
     ...
     feat.SetupFastBrief(FLAGS_image_files, idx, FLAGS_reduce_size);
    } else {
     //tracker.Process(image_array);
     int latest_knum = vo.keyframes()[1].frmno;
     //feat.ProcessFastBrief(FLAGS_image_files, idx, latest_knum,
    //                     FLAGS_reduce_size, feat.LastFtid());
     feat.ProcessFastBriefKey(FLAGS_image_files, idx, latest_knum,
                           FLAGS_reduce_size, feat.LastFtid());
    }
    ...
    */
    ...
    if (csio_out.IsOpen()) {
      ...
      for (int i = 0; i < ftids.size(); ++i) {
        ...
        //const KLTFeat& ft = feats[i];
        int prev_idx = FindInSortedArray(prev_ftids, ftids[i]);
...
LOG(INFO) << "rgb8 " <<  image.width() << ", " <<  image.height()
    ...
    if (feat.LatestKnum() != vo.keyframes()[1].frmno)
      prev_ftids = ftids;
    //prev_feats = feats;
  }
  end = clock();
  ...
  LOG(INFO) << "Process time : " << time << "s" << endl;
