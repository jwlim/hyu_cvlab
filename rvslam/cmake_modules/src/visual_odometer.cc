...
void VisualOdometer::MakeStereoPoints(const Mat3X& image_pts,
                                      Mat4X* stereo_pts) const {
  CHECK_NOTNULL(stereo_pts);
  const double fx = calib_.fx, fy = calib_.fy, cx = calib_.cx, cy = calib_.cy;
  stereo_pts->resize(4, image_pts.cols());
  for (int i = 0; i < image_pts.cols(); ++i) {
    const double nx = (image_pts(0, i) - cx) / fx;
    const double ny = (image_pts(1, i) - cy) / fy;
    const double nd = image_pts(2, i) / fx;
    stereo_pts->col(i) << nx, ny, nx + nd, ny;
  }
}
...
void VisualOdometer::MakeNormalizedPoints(const Mat3X& image_pts,
  ...
  const double fx = calib_.fx, fy = calib_.fy, cx = calib_.cx, cy = calib_.cy;
  const double k1 = calib_.k1, k2 = calib_.k2, k4 = 0;
...
void VisualOdometer::UpdateConnections(Keyframe* cur_frm, int num_keyframes) {
  map<Keyframe*, int> kfcounter;
  if (num_keyframes > world_->keyframes_.size())
    num_keyframes = world_->keyframes_.size();
  ...
  for (int i = 1; i < num_keyframes; ++i) {
    vector<int> kf_ftids;
    SetInliersFtids(world_->keyframes_[i], &kf_ftids);
    int pkf = CountMatchedFeatures(cur_ftids, kf_ftids);
    kfcounter.insert(make_pair(&world_->keyframes_[i], pkf));
  }
  ...
  int nmax = 0;
  const int th = FLAGS_covisible_thr;
